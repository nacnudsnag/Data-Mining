...
def main():
	...
	imp = Imputer(missing_values=2, strategy='mean', axis=0)  # missing encoded as 2
	print(X)                        # before the transformation
	X = imp.fit_transform(X)        # fit to data and transform
	print(X)                        # after the transformation
	...
	#Visualized Trees
	with open("dt1imputerer", 'w') as f:
		f = tree.export_graphviz(model1, out_file=f)
	with open("dt2imputerer", 'w') as f:
